...
export function buildWhatsAppCheckoutLink(
  ...
  items.forEach((item, index) => {
    ...
    lines.push(
      `   Rate: ${formatUSD(item.product.price)} / ${formatINR(item.product.price)}`
    )
    lines.push(`   Amount: ${formatUSD(lineTotal)} / ${formatINR(lineTotal)}`)
    lines.push('')
  })
  ...
  lines.push(`*Total: ${formatUSD(grandTotal)} / ${formatINR(grandTotal)}*`)
  lines.push('')
